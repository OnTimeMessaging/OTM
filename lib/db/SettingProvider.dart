...
class ScheduleProvider {
  ...
  set onNotificationTriggered(Function(Todo) onData) {
    ...
    _subMsgNotification =
        _ctrlNotification.stream.listen((todo) => onData(todo));
  }
  ...
  set onScheduleProcessed(Function(List<Todo>) onDone) {
    ...
    _subMsgs?.cancel();
    _subMsgs =
        _ctrlMsgs.stream.listen((todo) => onDone(todo));
  }
  ...
  //  // final todo = await TodoService.In.readTodos();
  ...
  //       DateTime.now().millisecondsSinceEpoch >=todo.executedAt)
  //       .forEach((Todo todo) {
